...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  realpath(root_dir.c_str(), abs_root_path);
  realpath(test_file.c_str(), abs_test_path);
  std::string root_path(abs_root_path);
  root_path.append("/");
  std::string test_path(abs_test_path);

  // Check if the test file is in the root_dir
  if (test_path.find(root_path) == test_path.npos) {
    return false;
  }
  // Checks that the test path is a subset of the root path
  if (root_path.size() >= test_path.size()) {
    return false;
  }
  return true;
}
...
string EscapeHtml(const string& from) {
  ...
  std::stringstream ss;
  int length = static_cast<int>(from.size());
