...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // Reads the bytes from the doctable, folds into crc
  for (int i = 0; i < doctable_bytes; i++) {
    unsigned char byte;
    if (fseek(f, sizeof(IndexFileHeader) + i, SEEK_SET) != 0) {
      return kFailedWrite;
    }
    int read = fread(&byte, sizeof(byte), 1, f);
    if (read != 1) {
      return kFailedWrite;
    }
  ...
  // Reads the bytes from the index, folds into crc
  for (int i = 0; i < memidx_bytes; i++) {
    unsigned char byte;
    if (fseek(f, sizeof(IndexFileHeader) + doctable_bytes + i, SEEK_SET) != 0) {
      return kFailedWrite;
    }
    int read = fread(&byte, sizeof(byte), 1, f);
    if (read != 1) {
...
static int WriteHTBucketRecord(FILE* f,
  ...
  BucketRecord bucket_rec = BucketRecord(num_elts, bucket_offset);
  ...
  // STEP 6.
  // Write the BucketRecord.
  if (fwrite(&bucket_rec, sizeof(bucket_rec), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    IndexFileOffset_t pos_offset =
                      record_pos + i * sizeof(ElementPositionRecord);
    if (fseek(f, pos_offset, SEEK_SET) != 0) {
      return kFailedWrite;
    }

    ElementPositionRecord record = ElementPositionRecord(element_pos);
    record.ToDiskFormat();
    ...
    int written = fn(f, element_pos, element);
    if (written == 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  return file_name_bytes + sizeof(header);
}
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  DocIDElementHeader header = DocIDElementHeader(doc_id, num_positions);
  header.ToDiskFormat();
  if (fseek(f, offset, SEEK_SET) != 0) {
    return kFailedWrite;
  }
  ...
  int position_offset = sizeof(header);
  Verify333(it != nullptr);
  for (int i = 0; i < num_positions; i++) {
    // STEP 13.
    // Get the next position from the list.
    LLPayload_t next_position;
    LLIterator_Get(it, &next_position);

    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    uint64_t pos64 = reinterpret_cast<uint64_t>(next_position);
    position.position = static_cast<uint32_t>(pos64);
    position.ToDiskFormat();
    if (fseek(f, offset + position_offset, SEEK_SET) != 0) {
      return kFailedWrite;
    }
    ...
    position_offset += sizeof(position);
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return ht_bytes + word_bytes + sizeof(header);
}
}  // namespace hw3
