...
// Takes in a LinkedList iterator and key, and attempts to find that
// key within the chain, returning true if found and false otherwise
// If the key is found, the iterator stays on that node
static bool FindKeyInChain(LLIterator *iter, HTKey_t key);

// Takes in a HashTable iterator and increments the bucket index
// until it reaches the next non-empty bucket. Returns true
// if it doesn't find a non-empty bucket, false otherwise
static bool IncrementIterBucket(HTIterator *iter);
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t *new_element = (HTKeyValue_t *)malloc(sizeof(HTKeyValue_t));

  // Ensure malloc did not fail
  Verify333(new_element != NULL);
  *new_element = newkeyvalue;

  // Creates iterator on current chain
  LLIterator *iter = LLIterator_Allocate(chain);

  if (LinkedList_NumElements(chain) == 0) {
    // The chain is empty, so the new keyvalue is added
    LinkedList_Push(chain, new_element);
    has_old = false;
    table->num_elements += 1;
  } else if (FindKeyInChain(iter, newkeyvalue.key)) {
    // Replaces the current keyvalue with the new one
    HTKeyValue_t *value;
    LLIterator_Get(iter, (LLPayload_t *) &value);
    *oldkeyvalue = *value;
    *value = newkeyvalue;
    free(new_element);
  } else {
    // The key was not found in the chain, so the keyvalue is added
    LinkedList_Push(chain, new_element);
    has_old = false;
    table->num_elements += 1;
  }
...
// Given a key and chain iterator, moves through the
// list until the key is found returning true. Returns false
// if the key was not found or the iterator is invalid
static bool FindKeyInChain(LLIterator *iter, HTKey_t key) {
  if (!LLIterator_IsValid(iter)) {
    return false;
  }
  // Gets current element without iterating
  HTKeyValue_t *find_element;
  LLIterator_Get(iter, (LLPayload_t *) &find_element);
  if ((*find_element).key != key) {
    // Loops through the iterator until the key is found
    while (LLIterator_Next(iter) && (*find_element).key != key) {
      LLIterator_Get(iter, (LLPayload_t *) &find_element);
      // If the key is found, do not iterate further
      if ((*find_element).key == key) {
        return true;
      }
    }
  }
  return (*find_element).key == key;
}
...
bool HashTable_Find(HashTable *table,
  ...
  // Creates iterator for current chain
  LLIterator *iter = LLIterator_Allocate(chain);
  bool found = false;

  // Iterates through chain until key is reached,
  // returns true if found, false otherwise
  if (FindKeyInChain(iter, key)) {
    HTKeyValue_t *get_element;
    LLIterator_Get(iter, (LLPayload_t *) &get_element);
    *keyvalue = *get_element;
    found = true;
  }
  LLIterator_Free(iter);
  return found;
...
bool HashTable_Remove(HashTable *table,
  ...
  // Finds the element in the chain and removes it
  ...
  if (FindKeyInChain(iter, key)) {
    ...
    *keyvalue = *get_element;
    ...
    // Free the iterator and old element
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // Returns true if the current bucket iterator
  // is not passed the end of a chain
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  if (HTIterator_IsValid(iter)) {
    // Moves to the next element in the chain
    if (LLIterator_Next(iter->bucket_it)) {
      return true;
    } else if (iter->bucket_idx < iter->ht->num_buckets) {
      // Moves to the next populated bucket in the table
      LLIterator *tmp = iter->bucket_it;
      LLIterator_Free(tmp);

      bool reached_end = IncrementIterBucket(iter);
      if (reached_end) {return false;}

      // Loops through the empty buckets until a non-empty one is found,
      // returns true if found, false otherwise
      while (LinkedList_NumElements(iter->ht->buckets[iter->bucket_idx]) < 1 \
             && iter->bucket_idx < iter->ht->num_buckets) {
        reached_end = IncrementIterBucket(iter);
        if (reached_end) {return false;}
      }
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  if (!HTIterator_IsValid(iter) || iter->ht->num_elements == 0) {
    ...
    HTKeyValue_t *get_element;
    LLIterator_Get(iter->bucket_it, (LLPayload_t *) &get_element);
    *keyvalue = *get_element;
  }
  return true;
}
...
static bool IncrementIterBucket(HTIterator *iter) {
  iter->bucket_idx += 1;
  if (iter->ht->num_buckets == iter->bucket_idx || \
      iter->ht->buckets[iter->bucket_idx] == NULL) {
    iter->bucket_it = NULL;
    return true;
  }
  return false;
